...
function initIncomeChart() {
  if (!Array.isArray(incomeData) || !incomeData.length) {
  ...
  if ($("#incomeLineChart").length) {
    ...
    gradientStroke.addColorStop(0, 'rgba(76, 175, 80, 0.6)');
    gradientStroke.addColorStop(1, 'rgba(76, 175, 80, 0.1)');
    ...
    new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Thu nhập',
          data: incomeData.map(item => item.y), // Sử dụng dữ liệu thực tế
        ...
        tooltips: {
          callbacks: {
            label: function(tooltipItem, data) {
              return tooltipItem.yLabel.toLocaleString('vi-VN') + ' đồng';
            }
          }
        },
        scales: {
  ...
  if ($("#expenseBarChart").length) {
    ...
    new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Chi tiêu',
          data: expenseData.map(item => item.y), // Sử dụng dữ liệu thực tế
...
function initDoughnutCharts() {
  ...
  if ($("#categoryDoughnutChart").length) {
    ...
    new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: categoryData.map(item => item.color || getRandomColor()),
          borderWidth: 0
        }]
      },
      options: {
    ...
    // Tạo labels bên ngoài biểu đồ
    var legendHtml = '<ul class="pie-chart-legend">';
    categoryData.forEach(function(item) {
      legendHtml += `<li>
        <span class="legend-dots" style="background-color: ${item.color || getRandomColor()}"></span>
        ${item.label} (${item.value}%)
      </li>`;
    });
    legendHtml += '</ul>';
    
    document.getElementById('chart-legend-container').innerHTML = legendHtml;
  }
}
